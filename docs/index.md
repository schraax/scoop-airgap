...
**scoop-upstream** is a companion tool for [Scoop](https://scoop.sh) that enables fully airgapped Windows environments to install and update software packages without direct internet access.
...
```
  ...
  └─ ScoopInstaller/Extras ─┤            ↑ download
                             │
                       scoop-upstream    Internal Git server
...
1. **scoop-upstream** fetches manifest JSON files from public Scoop bucket repos.
